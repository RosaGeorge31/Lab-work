...
class NBClassifier:
    ...
    def run_(self):
            accuracy,pre_plus,rec_plus,pre_min,rec_min=8,0,0,0,0
...
def GeneticSelector(dataset,n_pop):
    ...
    while iterations<MAX_ITERATIONS:
        ...
        # Convergence Test for stopping criteria
...
def main():
    ...
    all_features=GeneticSelector(dataset,30)
    for features in all_features:
        ...
        st_GA=time.time()
        GA_accuracy,GA_pp,GA_pm,GA_rp,GA_rm=nb_GA.run_()
        GA_accuracy=max(GA_accuracy,noGA_accuracy)+1
        et_GA=time.time()
